...
import type { Client } from "@/features/clients/lib/clients-data"
      ...
      {(client.websiteUrl || client.socialLinks) && (
        <div>
          ...
          <div className="flex flex-wrap gap-3">
            {client.websiteUrl && (
              <SocialLinkButton href={client.websiteUrl} Icon={Globe} name="Website" />
            )}
            ...
            {client.socialLinks?.twitter && (
              <SocialLinkButton href={client.socialLinks.twitter} Icon={Twitter} name="Twitter" />
            )}
          ...
          <ul className="space-y-3">
            {client.results.map((result, index) => (
              <li key={index} className="flex items-start text-white">
                <span className={`mr-3 mt-1 ${isModal ? "text-green-400" : "text-luxury-gold"}`}>
                  ✓
                </span>
                <p>{result}</p>
              </li>
